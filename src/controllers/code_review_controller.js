...
const reviewProgramCode = async (req, res) => {
  try {
    let combinedCode = "";
    const { code } = req.body;
    ...
    const result = await ai_service.reviewCode(finalCode);
...
const suggestedProgramCodeFixes = async (req, res) => {
  const { code } = req.body;

  try {
    const result = await ai_service.suggestCodeFixes(code);
